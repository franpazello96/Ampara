...
export default function FinancialDonator() {
  ...
  const normalDonations = donations.filter(d => !d.recurrence);
  const recurringDonations = donations.filter(d => d.recurrence);
  ...
  const handleEditRecurrence = async (id: number, updated: Partial<Donation>) => {
    ...
    try {
      const response = await fetch(`https://localhost:5001/api/donation/update/${id}`, {
        method: "PUT",
        headers: {
